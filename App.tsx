                   ...
                   <div className="space-y-8 animate-reveal">
                      <div className="flex flex-col sm:flex-row items-center justify-between bg-indigo-50 p-6 rounded-[2rem] border border-indigo-100 gap-4">
                         ...
                         <div className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${
                           matchResults.type === 'exact' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
                         }`}>
                ...
                <div className="flex flex-wrap gap-2 justify-end w-full">
                  {['Semua', 'PTN', 'PTS', 'PTLN'].map((cat) => (
                    <button
                      ...
                      className={`flex-1 md:flex-none px-6 py-3 rounded-2xl text-[10px] font-black transition-all border-2 ${
                        (filterCategory === 'All' ? 'Semua' : filterCategory) === cat ? 'bg-indigo-600 text-white border-indigo-600 shadow-xl shadow-indigo-100' : 'bg-white text-slate-400 border-slate-100 hover:border-indigo-300'
                      }`}
                    >
                      {cat}
                    </button>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end items-center bg-slate-50 p-2 rounded-2xl border border-slate-200 animate-reveal w-full md:w-auto overflow-x-auto no-scrollbar">
                    ...
                    {['Semua', ...getSubPaths(filterCategory)].map(path => (
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-xl text-[10px] font-bold transition-all whitespace-nowrap ${
                          (filterPath === 'All' ? 'Semua' : filterPath) === path ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-200'
                        }`}
                      >
                        {path}
                      </button>
         ...
         >
            {menuItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-3 px-6 md:px-8 py-4 rounded-[1.5rem] transition-all duration-300 shrink-0 group ${
